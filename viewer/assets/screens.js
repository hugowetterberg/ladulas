...
export function home(state, go) {
  ...
  if (!peers.length && !pairings.length) {
    body.push(ui.empty("No machines yet",
      "Pair with a machine and it can ask this one for approvals. Run "
      + "`ladulas pair` on either side; the other reads the code."));
  }
...
export function keys(state) {
  ...
  if (!held.length) {
    body.push(ui.empty("No keys",
      "`ladulas keys generate` makes one. Keys are the daemon's — this window "
      + "shows them and never holds one (decision Z)."));
  }
...
export async function peer(state, fingerprint, go) {
  ...
  ui.note("The fingerprint is what the two machines compared when they paired. "
    + "Revoking the pairing is `ladulas peers forget` — a window that could do "
    + "it is a window a stray click can unpair a machine from.")));
  ...
  body.push(ui.note("Read from " + found.name + " just now, or read from it once "
    ...
    + "anything about any signature (§6)."));
...
export function settings(state) {
  ...
  body.push(ui.card(null,
    ...
    ui.note("Locking suspends approving here without forgetting anything, and a "
      ...
      + "key from memory: the agent offers nothing until it is unlocked (§10)."),
    state.lockControls()));
  ...
  body.push(ui.card(null,
    ui.note("This window draws what the daemon says and answers for it; the "
      + "keys, the agent socket and the approval engine are its (decision Z)."),
