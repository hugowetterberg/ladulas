...
**A document over the per-file cap is served cut short, not refused.** The
cap used to mean "not offered": the file was listed as unreadable with a
reason, and the sync neither sent it nor took away an older copy. That is
the right answer for a kind nobody can render and the wrong one for the
case it kept hitting — a long document. This repository's own
`architecture.md` was three hundred kilobytes against a cap of two
hundred and fifty-six, so the file every other document tells the reader
to go and read was the one they could not open, and once the picker
listed what had been *synced* rather than what the publisher *had*, it
did not appear at all: no entry, no reason, nothing to notice. The cap is
about what a machine will send, not about what a reader may see. So the
cap is a megabyte, and a file past it is cut back to the last line ending
before it — a line ending because a markdown parser handed half a fenced
block produces a document that is wrong rather than short — with a
lookback bounding the search, past which the cut falls where the cap
does and the last line is ragged. The entry says it was cut and how big
the whole file is, and the reader is told in a line above the text.
**Only text is cut**: half of anything else is not a shorter version of
it, so a kind that is not text is still refused at its full size.
...
before you can refuse it. The colours are the sixteen ANSI ones, so red and
green are whatever the terminal's owner has decided those are, and every
coloured line says in words what the colour says. And the log goes to stderr
only when stderr is not the screen: one INFO line about attaching is enough
to put a timestamp through the middle of a card.

**The change is three screens and not one** (decision AN). The card is the
facts and a list of the files; `f` opens that list, where typing narrows it;
and one file's hunks are a screen of their own. The window folds the hunks
into the card as disclosures and this does not, for two reasons that only
show up in a terminal. A card whose length depends on the shape of the change
pushes the four facts a decision rests on off the screen on any commit worth
reading — the very thing the pinned answer line exists to prevent. And a
screen somebody is *in* is what makes `enter` usable for the answer given
most often: on the card it approves, and while somebody is reading a change
it closes the change, because a signature is not a thing to approve by
reflex. The letters still answer from inside a change, being deliberate.

It went the other way first, and briefly: the hunks were folded into the card
behind disclosures with a cursor beside the files, `enter` toggling the one
under it. Three things were wrong at once and each alone would have been
enough. The keys that moved the cursor were `n` and `p`, which appeared in
the help table and nowhere else, while the arrow keys scrolled and
`left`/`right` moved between *waiting requests* — so on the ordinary card,
with one request waiting, every arrow key looked broken and the second file
of a change could not be reached at all. The first `enter` was spent placing
the cursor rather than opening anything, so it produced a marker beside a
file that stayed shut. And one set of keys that sometimes scrolls and
sometimes walks a cursor is two programs sharing a keyboard, which is the
thing to avoid rather than the thing to tune. **Do not put the cursor back
into the card.**
